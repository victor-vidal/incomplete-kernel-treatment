...
for iteration in range(5):
    for technique in ['zero', 'mean', 'median', 'isvd', 'knn']:
        for percentage in [10, 30, 50, 70]:
            ...
            # Create directories
            if not os.path.exists(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}'):
                os.makedirs(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}')
            ...
            for kd in kd_file_names:
                with open(f'{expirement_data_path}/imputed_kernels/{iteration}/{technique}/{percentage}/{kd}', 'r') as f:
                    kd_list.append(np.loadtxt(f))


            # Cell line kernels
            # Read file names of cell line kernels
            with open(complete_data_path + '/Cell_line_kernels/Cell_kernel_file_names.txt', 'r') as f:
            ...
            for kc in kc_file_names:
                with open(f'{expirement_data_path}/imputed_kernels/{iteration}/{technique}/{percentage}/{kc}', 'r') as f:
            ...
            with open(complete_data_path + '/Labels.txt', 'r') as f:  # rows: drugs, columns: cell lines
                Y = np.loadtxt(f)
            ...
            outer_folds = np.loadtxt(f'{expirement_data_path}/cross_validation/{iteration}/outer_folds.txt').astype(int)
            ...
            # Predicted drug responses
            np.savetxt(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/y_pred_vec.txt', y_pred_outer_vec, delimiter='\t')  

            # In the below files, each row corresponds to the result from a single outer CV fold   
            # RMSE
            np.savetxt(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/RMSE.txt', rmse_outer, delimiter='\t')  
            # Pearson correlation
            np.savetxt(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/Pearson_correlation.txt', pearson_outer, delimiter='\t')  
            # F1 score
            np.savetxt(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/F1_score.txt', f1_outer, delimiter='\t')  
            # Optimal value of the regularization parameter \lambda 
            np.savetxt(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/selected_lambda.txt', model, delimiter='\t')
            # Pairwise kernel weights
            np.savetxt(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/pairwise_kernel_weights.txt', k_weights_outer, delimiter='\t')  

            # File names of the corresponding pairwise kernels, in the same order as in the file
            # with pairwise kernel weights 
            thefile = open(f'{expirement_data_path}/results/{iteration}/{technique}/{percentage}' + '/pairwise_kernel_names.txt', 'w')
            for i in range(P):
                thefile.write("%s_KRONECKER_%s\t" %(kd_file_names[kd_ids[i]], kc_file_names[kc_ids[i]]))
            thefile.close()


            print(f'\nSuccess! Iteration: {iteration}, Technique: {technique}, Percentage: {percentage}.') 