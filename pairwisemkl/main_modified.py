...
imputed_data_path = '../imputed_kernels'
imputed_data_results_path = '../imputed_kernels_results'


for technique in ['zero', 'mean', 'median', 'isvd', 'knn'][4:]:
    for percentage in [10, 30, 50, 70]:
        ...
        for kd in kd_file_names:
            with open(imputed_data_path + f'/{technique}/{percentage}/' + kd, 'r') as f:
        ...
        for kc in kc_file_names:
            with open(imputed_data_path + f'/{technique}/{percentage}/' + kc, 'r') as f:
        ...
        outer_folds = np.loadtxt(complete_data_path + '/Folds/outer_folds.txt').astype(int)
        ...
        # Predicted drug responses
        np.savetxt(imputed_data_results_path + f'/{technique}/{percentage}' + '/y_pred_vec.txt', y_pred_outer_vec, delimiter='\t')  

        # In the below files, each row corresponds to the result from a single outer CV fold   
        # RMSE
        np.savetxt(imputed_data_results_path + f'/{technique}/{percentage}' + '/RMSE.txt', rmse_outer, delimiter='\t')  
        # Pearson correlation
        np.savetxt(imputed_data_results_path + f'/{technique}/{percentage}' + '/Pearson_correlation.txt', pearson_outer, delimiter='\t')  
        # F1 score
        np.savetxt(imputed_data_results_path + f'/{technique}/{percentage}' + '/F1_score.txt', f1_outer, delimiter='\t')  
        # Optimal value of the regularization parameter \lambda 
        np.savetxt(imputed_data_results_path + f'/{technique}/{percentage}' + '/selected_lambda.txt', model, delimiter='\t')
        # Pairwise kernel weights
        np.savetxt(imputed_data_results_path + f'/{technique}/{percentage}' + '/pairwise_kernel_weights.txt', k_weights_outer, delimiter='\t')  

        # File names of the corresponding pairwise kernels, in the same order as in the file
        # with pairwise kernel weights 
        thefile = open(imputed_data_results_path + f'/{technique}/{percentage}' + '/pairwise_kernel_names.txt', 'w')
        for i in range(P):
